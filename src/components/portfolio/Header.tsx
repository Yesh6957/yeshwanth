      ...
      <div className="container mx-auto px-4 xl:px-8">
        <div className="flex items-center justify-between h-16 xl:h-20">
          <button
            ...
            className="text-2xl xl:text-3xl font-bold text-primary hover:text-accent smooth-transition"
          >
            Yesh
          </button>
          ...
          <nav className="container mx-auto px-4 py-4 flex flex-col gap-2">
            {navLinks.map((link) => (
              <button
                key={link.name}
                onClick={() => scrollToSection(link.href)}
                className="text-left px-4 py-3 rounded-lg hover:bg-secondary smooth-transition font-medium"
              >
                {link.name}
              </button>
            ))}
