...
function useContainerScrollContext() {
  ...
  if (!context) throw new Error("useContainerScrollContext must be used within a ContainerScrollContextProvider")
  return context
}
...
export const CardTransformed = React.forwardRef<HTMLDivElement, CardStickyProps>(
  (
    ...
    const rotateRange = [range[0] - 0.5, range[1] / 1.5]

    const y = useTransform(scrollYProgress, range, ["0%", "-50%"])
    const rotate = useTransform(scrollYProgress, rotateRange, [incrementRotation / 2, 0])
    const smoothY = useSpring(y, { damping: 25, stiffness: 120 })
    const smoothRotate = useSpring(rotate, { damping: 25, stiffness: 120 })
    const transform = useMotionTemplate`
      translateZ(${index * incrementZ}px)
      translateY(${smoothY})
      rotate(${smoothRotate}deg)
    `

    const dx = useTransform(scrollYProgress, rotateRange, [2, 0])
    const dy = useTransform(scrollYProgress, rotateRange, [2, 6])
    const blur = useTransform(scrollYProgress, rotateRange, [2, 12])
    const alpha = useTransform(scrollYProgress, rotateRange, [0.08, 0.15])
    const filter = variant === "light" ? useMotionTemplate`drop-shadow(${dx}px ${dy}px ${blur}px rgba(0,0,0,${alpha}))` : "none"

    return (
      <motion.div
        layout="position"
        ref={ref}
        style={{ top: index * incrementY, transform, backfaceVisibility: "hidden", zIndex: (arrayLength - index) * incrementZ, filter, ...style }}
...
export const ReviewStars = React.forwardRef<HTMLDivElement, ReviewProps>(({ rating, maxRating = 5, className, ...props }, ref) => {
    ...
    <div className={cn("flex items-center gap-2", className)} ref={ref} {...props}>
      <div className="flex items-center">
        {[...Array(filledStars)].map((_, i) => (
          <svg key={i} className="size-4 text-inherit" fill="currentColor" viewBox="0 0 20 20">
        ...
        {[...Array(emptyStars)].map((_, i) => (
          <svg key={i} className="size-4 text-gray-300" fill="currentColor" viewBox="0 0 20 20">
            <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.286 3.957a1 1 0 00.95.69h4.162c.969 0 1.371 1.24.588 1.81l-3.37 2.448a1 1 0 00-.364 1.118l1.287 3.957c.3.921-.755 1.688-1.54 1.118l-3.37-2.448a1 1 0 00-1.175 0l-3.37 2.448c-.784.57-1.838-.197-1.54-1.118l1.287-3.957a1 1 0 00-.364-1.118L2.05 9.384c-.783-.57-.38-1.81.588-1.81h4.162a1 1 0 00.95-.69l1.286-3.957z" />
          </svg>
        ))}
      </div>
